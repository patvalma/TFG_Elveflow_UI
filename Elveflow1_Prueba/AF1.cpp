...
int main_AF1()
{
	...
	do {// loop until exit
		...
		if (answer == "get pressure")
		{
			...
			AF1_Get_Press(MyAF1_ID, 1000, Calibration, &get_pressure, 1000);//pass pointer to get_pressure in order to modify it's value
			cout << "pressure: " << get_pressure << " mbar" << endl; //mue muestra por pantalla la presion que he puesto
		}
